...
const Faqs: React.FC = () => {
  ...
  return (
    <div className="container xl:max-w-[1180px]" id="faq">
      ...
      <Paragraph
        textName="You have got Question. We have got Answer"
        className="font-normal text-sm md:text-base text-center mt-2 md:mt-4"
      />
      <div className="lg:max-w-[975px] max-w-[740px] mx-auto mt-5 lg:mt-[60px]">
        {FaqList.map((data: FaqData, index: number) => (
          <div
            key={index}
            className={`overflow-hidden transition-all duration-500 ease-in-out rounded-lg lg:rounded-2xl ${
              activeIndex === index
          ...
          >
            <button
              ...
              } w-full text-left p-3 lg:p-[21px] transition duration-300 ease-in-out relative`}
            >
              <div className="flex justify-between items-center gap-2">
                <span
                  className={`${
                    activeIndex === index ? "text-white" : "text-black"
                  } text-sm md:text-lg xl:text-md font-medium transition-colors duration-300`}
                >
                  {data.question}
                </span>
                <span className="float-right transition-transform duration-300 ease-in-out">
                  <div
                    className={`size-5 md:size-6 flex justify-center items-center rounded-full ${
                      activeIndex === index ? "bg-white" : "bg-blue"
                    ...
                    >
                      <path
                        className={`transition-colors duration-300 ${
                          activeIndex === index ? "fill-blue" : "fill-white"
                        }`}
            ...
            >
              <div
                className={`p-3 lg:p-6 transition-opacity duration-300 ${
